...
public:
    ...
    {
        if (cudaMalloc(&_ptr, size * sizeof(uint32_t)) != cudaSuccess)
    ...
    {
        if (cudaMemcpy(_ptr, data.data(), size * sizeof(uint32_t), cudaMemcpyHostToDevice) != cudaSuccess)
            throw std::runtime_error("Failed to copy data to device memory!");
    }

    void read(std::array<uint32_t, size>& buffer)
    {
        if (cudaMemcpy(buffer.data(), _ptr, size * sizeof(uint32_t), cudaMemcpyDeviceToHost) != cudaSuccess)
