...
def uniform_and_state_update(rng_key: jnp.array, count: int) -> Tuple[jnp.array, jnp.array]:
    ...
    next_rng_key = increase_counter(rng_key, num_blocks) # skipping only half as far (num_blocks/2) passes small crush (but /3 or /4 do not) .. test again with more extensive test suites
